  ...
  useEffect(() => {
    ...
    const unsubscribe = peer.subscribe((event: PeerEvent) => {
      ...
      if (["fixture.updated", "room.state", "room.details"].includes(event.type) && revisionTimer.current === null) {
        ...
        // Historical fixture replay can advance several rooms at once. A
        // two-second refresh cadence keeps match presentation current without
        // continuously redrawing the whole native accessibility tree.
        }, 2_000);
      }
...
function Home({ peer, session, onSession, onSettings, onOpenRoom }: { peer: ReturnType<typeof usePeer>; session: Json | null; onSession(value: Json | null): void; onSettings(): void; onOpenRoom(roomId: string): void }) {
  ...
  const matchday = selectMobileMatchdayFocus(fixtures, rooms);
  const matchdayCard = matchday.fixture as Json | null;
  const matchdayFixture = matchdayCard?.fixture as Json | undefined;
  const openMatchday = () => {
    if (matchday.room?.room?.id != null) {
      onOpenRoom(String(matchday.room.room.id));
      return;
    }
    if (matchdayFixture?.id == null) return;
    setSelectedFixture(String(matchdayFixture.id));
    setRoomName(`${matchdayFixture.home.name} × ${matchdayFixture.away.name}`);
    setMode("create");
  };
  ...
  return (
    <ScrollView refreshControl={<RefreshControl refreshing={false} onRefresh={load} tintColor={colors.ink} />} contentContainerStyle={styles.home} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.mobileIdentityRow}>
        <PeerAvatar userId={session.userId} displayName={session.displayName} size="sm" isCurrentUser />
        <View style={styles.flex}><Text style={styles.eyebrow}>YOUR MATCHDAY</Text><Text style={styles.mobileIdentityName} numberOfLines={1}>{session.displayName}</Text></View>
      </View>

      {matchdayCard && matchdayFixture ? (
        <Pressable accessibilityRole="button" accessibilityLabel={`${matchday.room ? "Return to" : "Set up"} ${matchdayFixture.home.name} versus ${matchdayFixture.away.name}`} onPress={openMatchday} style={styles.matchdayHero}>
          <View style={styles.matchdayTop}><Text style={styles.matchdayStatus}>{matchdayCard.phase === "live" ? `● LIVE ${matchdayCard.minute ?? ""}′` : "NEXT SIGNED FIXTURE"}</Text><Text style={styles.matchdayCompetition} numberOfLines={1}>{matchdayFixture.competition}</Text></View>
          <Text style={styles.matchdayTeams}>{matchdayFixture.home.name} <Text style={styles.matchdayVersus}>vs</Text> {matchdayFixture.away.name}</Text>
          <Text style={styles.matchdayScore}>{matchdayCard.score ? `${matchdayCard.score.home}–${matchdayCard.score.away}` : new Date(Number(matchdayFixture.kickoff)).toLocaleString()}</Text>
          <View style={styles.matchdayCta}><Text style={styles.matchdayCtaText}>{matchday.room ? "RETURN TO THE LIVE ROOM" : matchdayCard.phase === "live" ? "START A ROOM FOR THIS MATCH" : "SET UP THIS MATCH ROOM"}</Text><Text style={styles.matchdayCtaArrow}>›</Text></View>
        </Pressable>
      ) : (
        <View style={styles.matchdayHero}><Text style={styles.matchdayStatus}>SIGNED FIXTURE FEED</Text><Text style={styles.matchdayTeams}>The next match has not arrived yet.</Text><Text style={styles.matchdayEmpty}>FullTime will not invent a schedule. You can still join a room with a verified invite.</Text></View>
      )}

      <Pressable accessibilityRole="button" onPress={() => setMode("join")} style={styles.homeAction}><Text style={styles.homeActionEyebrow}>HAVE AN INVITE?</Text><Text style={styles.homeActionTitle}>Join a private room</Text><Text style={styles.homeActionArrow}>›</Text></Pressable>
  ...
  return <View style={[styles.messageRow, mine && styles.messageRowMine]}>
    ...
    <View style={[styles.messageColumn, mine && styles.messageColumnMine]}>
      ...
      <View style={[styles.message, mine && styles.messageMine]}>
        {item.quote ? <View style={styles.messageQuote}><Text style={styles.messageQuoteAuthor}>{String(item.quote.author?.displayName ?? "Room member")}</Text><Text numberOfLines={2} style={styles.messageQuoteText}>{String(item.quote.text ?? "")}</Text></View> : null}
...
const styles = StyleSheet.create({
  ...
  onboarding: { flexGrow: 1, justifyContent: "center", padding: 28, gap: 18 }, home: { padding: 20, paddingBottom: 60, gap: 14 }, focusFlow: { padding: 20, paddingBottom: 60, gap: 16 }, flowHeader: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 8 }, flowBack: { width: 38, height: 38, alignItems: "center", justifyContent: "center", borderRadius: 19, borderWidth: 1, borderColor: colors.ash }, flowBackText: { color: colors.ink, fontSize: 28, lineHeight: 30 }, homeAction: { minHeight: 96, borderRadius: 20, borderWidth: 1, borderColor: colors.ash, backgroundColor: "rgba(251,250,247,0.62)", padding: 18, justifyContent: "center" }, homeActionEyebrow: { color: colors.smoke, fontFamily: mono, fontSize: 8, letterSpacing: 1.2 }, homeActionTitle: { color: colors.ink, fontFamily: serif, fontSize: 25, marginTop: 4 }, homeActionArrow: { position: "absolute", right: 18, bottom: 16, color: colors.blue, fontSize: 20 }, topRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }, headerActions: { flexDirection: "row", alignItems: "center", gap: 5 }, settingsButton: { width: 36, height: 36, borderRadius: 18, borderWidth: 1, borderColor: colors.ash, alignItems: "center", justifyContent: "center" }, settingsGlyph: { color: colors.ink, fontSize: 17 }, settingsHeader: { minHeight: 86, paddingHorizontal: 20, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: colors.ash }, settingsContent: { padding: 20, paddingBottom: 60, gap: 14 }, accountId: { color: colors.graphite, fontFamily: mono, fontSize: 10, lineHeight: 16 }, brand: { flexDirection: "row", alignItems: "center", gap: 7, flexShrink: 1 }, wordmark: { color: colors.ink, fontFamily: serif, fontSize: Platform.OS === "android" ? 25 : 30, fontWeight: "700" }, eyebrow: { color: colors.smoke, fontFamily: mono, fontSize: 10, letterSpacing: 1.7 }, hero: { color: colors.ink, fontFamily: serif, fontSize: 42, lineHeight: 45, textAlign: "center" }, heading: { color: colors.ink, fontFamily: serif, fontSize: 34 }, subheading: { color: colors.ink, fontFamily: serif, fontSize: 25, marginTop: 3 }, body: { color: colors.ink, fontSize: 14, lineHeight: 21 }, bodyMuted: { color: colors.smoke, fontSize: 13, lineHeight: 19 }, transport: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: colors.ash, borderRadius: 20, paddingHorizontal: 8, paddingVertical: 6 }, transportText: { color: colors.graphite, fontFamily: mono, fontSize: 9, textTransform: "uppercase" }, dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: colors.gold, marginRight: 5 }, dotOnline: { backgroundColor: colors.green },
  mobileIdentityRow: { minHeight: 46, flexDirection: "row", alignItems: "center", gap: 10 }, mobileIdentityName: { color: colors.ink, fontFamily: serif, fontSize: 18, marginTop: 1 }, matchdayHero: { minHeight: 260, overflow: "hidden", borderRadius: 22, backgroundColor: colors.ink, padding: 20, justifyContent: "space-between" }, matchdayTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 }, matchdayStatus: { color: "#F3B7B7", fontFamily: mono, fontSize: 9, letterSpacing: 1 }, matchdayCompetition: { flex: 1, color: colors.ash, fontFamily: mono, fontSize: 9, textAlign: "right" }, matchdayTeams: { color: colors.white, fontFamily: serif, fontSize: 31, lineHeight: 36 }, matchdayVersus: { color: colors.smoke }, matchdayScore: { color: colors.white, fontFamily: mono, fontSize: 21 }, matchdayEmpty: { color: colors.ash, fontSize: 13, lineHeight: 20 }, matchdayCta: { minHeight: 48, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.18)", paddingTop: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, matchdayCtaText: { flex: 1, color: colors.white, fontFamily: mono, fontSize: 9, letterSpacing: 0.8 }, matchdayCtaArrow: { color: colors.white, fontSize: 24 },
  peerPreview: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1, borderColor: colors.ash, borderRadius: 18, backgroundColor: "rgba(251,250,247,0.72)", padding: 14 }, peerPreviewCopy: { flex: 1, minWidth: 0, gap: 4 }, peerPreviewName: { color: colors.ink, fontFamily: serif, fontSize: 22 }, reshuffleButton: { minHeight: 36, borderRadius: 18, borderWidth: 1, borderColor: colors.ash, paddingHorizontal: 12, alignItems: "center", justifyContent: "center" }, reshuffleText: { color: colors.ink, fontFamily: mono, fontSize: 10, letterSpacing: 1 }, memberAvatar: { marginRight: 4 },
