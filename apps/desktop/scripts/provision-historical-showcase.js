...
async function main () {
  ...
  const [{ FixturePlanePublisher }, { AuthenticatedFixtureReplay, loadAuthenticatedFixtureArchive }, { createLogger }, { AnswerAttestorService }, manifestModule] = await Promise.all([
  ...
  const modules = { FixturePlanePublisher, AuthenticatedFixtureReplay, loadAuthenticatedFixtureArchive, createLogger, AnswerAttestorService }
  ...
  try {
    ...
    if (pending.length) {
      ...
      try {
        ...
        for (const fixture of pending) {
          const replay = new AuthenticatedFixtureReplay(await loadAuthenticatedFixtureArchive(fixture.archivePath))
    ...
    publicNetwork = await openNetwork(modules, corpus, persisted, undefined, () => attestorTime)
...
async function openNetwork (modules, corpus, persisted, bootstrap, attestorClock) {
