/**
 * Deterministic author chrome so room members read as people, not grey chips.
 * Color + geometry from userId so every peer renders the same avatar everywhere.
 */

import { hashSeed } from "@/lib/peer-identity";
...
/** Geometry knobs for PeerAvatar SVG (stable per seed). */
