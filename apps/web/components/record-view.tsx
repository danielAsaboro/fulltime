...
export function RecordView() {
  ...
  return <Container className="py-12"><div className="mx-auto max-w-4xl space-y-10"><ShareCard eyebrow="Local Pear record" title={value.displayName} stats={[{ label: "Fan IQ", value: value.fanIq.toLocaleString() }, { label: "Accuracy", value: `${Math.round(value.accuracy * 100)}%` }, { label: "Matches", value: String(value.matchesPlayed) }]} tagline="Only accepted answers whose pinned receipts are available here." /><div className="flex flex-wrap items-center gap-3"><CopyLinkButton /><p className="font-mono text-caption text-smoke">{value.totalCalls} accepted answers · receipts remain room-scoped</p></div><section className="space-y-5"><h2 className="text-heading-sm text-off-black">Your answers</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{value.entries.map((entry) => <EntryCard key={entry.receiptId} entry={entry} />)}</div></section></div></Container>;
}
