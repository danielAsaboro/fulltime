import type { FanIqView } from "@/lib/data";
...
export function FanIqStrip({ iq, className }: { iq: FanIqView; className?: string }) {
  ...
  return (
    <div className={cn("grid grid-cols-3 gap-4 rounded-lg border border-ash bg-parchment px-5 py-4", className)}>
      ...
      <Cell
        ...
        sub={scored ? `${iq.correctCalls}/${iq.scoredCalls}` : "no calls yet"}
      />
      <Cell
        label="Room rank"
        value={iq.roomRank > 0 ? `#${iq.roomRank}` : "—"}
        sub={iq.roomSize ? `of ${iq.roomSize.toLocaleString()}` : undefined}
      />
    </div>
  );
}
